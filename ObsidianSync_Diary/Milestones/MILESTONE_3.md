...
|---|---|---|
|Velocità di acquisizione know-how|9/10|JUnit 5, Mockito, AssertJ assorbiti in un'unica milestone. Pattern ARRANGE/ACT/ASSERT applicato con consistenza da subito.|
|Velocità di delivery|8.5/10|Suite da 32 test completata autonomamente. Rallentamenti su problemi ambientali (JDK, ByteBuddy) non imputabili a lacune tecniche.|
|Impegno|9/10|Ha investigato autonomamente ogni errore prima di chiedere. Ha segnalato criticità (setTimestamp, costruttore di test) invece di ignorarle.|
|Qualità del codice|8.5/10|Naming descrittivo, helper estratti correttamente, pattern coerenti. Punti di miglioramento: catch silenzioso nel setup, drain della coda, stile AssertJ inconsistente. Tutti corretti in revisione.|

### Note del Trainer

La milestone 3 è stata la più tecnica fino ad ora — non per la complessità del dominio ma per la densità degli strumenti nuovi introdotti simultaneamente. Il candidato ha dimostrato di saper leggere i messaggi di errore e risalire alla causa radice in modo autonomo, senza aspettare l'imbeccata.

Il dettaglio più significativo: ha riconosciuto autonomamente che `setTimestamp` pubblico era un code smell, lo ha segnalato nel codice con una nota, e ha poi implementato la soluzione ottimale (costruttore package-private) quando la briciola è stata espansa. Questo è esattamente il tipo di sensibilità alla qualità che si vuole vedere in un candidato senior.

**Aree su cui lavorare nella Milestone 4:**

- Integrazione con Task Scheduler — configurazione dei tre task su Windows
- Test end-to-end su vault reale in ambiente controllato
- `PUSH_MANUAL` come quarto EventType — collegamento con la tray icon
- Considerare `NetworkException` vs `GitException` per differenziare retry e no-retry in `GitService`